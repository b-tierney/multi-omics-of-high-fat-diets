...
### LOAD LIPID DATA
metabolites = read.csv('data_packet/metabolites/Diet_lipidomics_batch2.txt',sep='\t') %>% filter(Time == 'M8') %>% select(-Time) %>% mutate(rawval = 10^(ValueLog10))  %>% dplyr::rename(Metabolite = Compound) %>% select(Metabolite,SampleID,ValueLog10,Diet,Type,rawval)
metabolites$SampleID = gsub('_M8_2_19_2020','',metabolites$SampleID )
metabolites$SampleID = gsub('_M8_2_20_2020','',metabolites$SampleID )
metabolites$SampleID = gsub('_M8_2_21_2020','',metabolites$SampleID )
metabolites$SampleID = gsub('_M8_2_18_2020','',metabolites$SampleID )
...
ggplot()+geom_point(data = site_scores %>% data.frame,size=2, aes(x = dbRDA1, y = dbRDA2,color=DIET)) +theme_minimal() + labs(title = "Lipids", x = "RDA1", y = "RDA2")+ theme(legend.position = 'bottom') + geom_segment(data = biplot_scores %>% data.frame,size=.5, aes(x = 0, y = 0, xend = dbRDA1*1, yend = dbRDA2*1,color = var), arrow = arrow(length = unit(0.1, "inches")))+stat_ellipse(data = site_scores %>% data.frame, aes(x = dbRDA1, y = dbRDA2,color = DIET)) + scale_color_brewer(palette = 'Set1') + scale_color_manual(values = colorsDiet)
ggsave('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/biplot_dbrda_LIPIDS.pdf',width=6,height=6)
...
write.csv(output,'association_output/metabolic_pathway_regression_output_dietLIPIDS.csv')
...
date()
print('Finished linear modeling...')
write.csv(output,'association_output/metabolic_metaphlan4_regression_output_dietLIPIDS.csv')
...
output2 = output2 %>% purrr:::reduce(left_join,by='features')
date()
print('Finished linear modeling...')

all_output = output2 %>% filter(features!='(Intercept)') %>% column_to_rownames('features')
write.csv(all_output,'association_output/metabolic_metaphlan4_regression_output_lassoLIPIDS.csv')

date()
output = purrr::map(unique(metabolites2$Metabolite), function(x) regress_microbe_metabolite3(x,merged_data_tpm_avg %>% mutate(PATHWAY = SGB) ,metabolites2 %>% select(-SEX,-rawval,-Type),minval)) 
output2 = purrr::map(output, function(x) if(typeof(x)!='character'){return(x)})
output2 = output2[which(!sapply(output2, is.null))]
...
write.csv(output2,'~/HMS Dropbox/Braden Tierney/mouse_diet_semir/association_output/metabolic_metaphlan4_regression_output_pairwiseLIPIDS.csv')

#### PLOT METABOLITE OUTPUT BY DIET VOLCANO
res = read.csv('association_output/metabolic_metaphlan4_regression_output_dietLIPIDS.csv') %>% mutate(direction = if_else(estimate>0,'positive',if_else(estimate<0,'negative',NA)))
...
ggsave('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/METABOLITE_diet_metabolite_association_countsLIPIDS.pdf',width=3,height=2.5)
...
#pdf('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/METABOLITE_upset_dietLIPIDS.pdf',width=6,height=4)
...
col_fun = circlize::colorRamp2(c(-5,-.001,0,.001,5), (c('darkblue','lightblue','white','pink',"#d73027")))

pdf('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/metabolite_heatmap_unlabeled_clusteredLIPIDS.pdf',width=14,height=3)
Heatmap(merged_data_cd_sub_wide  %>% column_to_rownames('Metabolite') %>% t,col=col_fun,cluster_columns = T,column_names_rot = 60)
dev.off()
...
metphaphlanlasso = read.csv('association_output/metabolic_metaphlan4_regression_output_lassoLIPIDS.csv',check.names=F) %>% melt() 
...
ggsave('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/bacteria_with_most_metabolites_metaphlan4LIPIDS.pdf',width=16,height=5)

bacregout = read.delim('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/association_output/metabolic_metaphlan4_regression_output_pairwiseLIPIDS.csv',sep=',')  %>% mutate(BH = p.adjust(p.value))
...
ggsave('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/bacteria_with_most_metabolites_metaphlan4_FDRLIPIDS.pdf',width=10,height=4)


#metphaphlanlasso  = metphaphlanlasso %>% select(-all_of(colnames(metphaphlanlasso)[grepl("^X-", colnames(metphaphlanlasso))]))
#pdf('~/HMS Dropbox/Braden TierneySuper Mice Diet Omics Project/Metagenomics/Metabolomic_Associations_Nov23/metabolite_microbe_heatmapLIPIDS.pdf',width=16,height=10)
...
pdf('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/metabolite_diet_associations_hmLIPIDS.pdf',width=5,height=12)
...
pdf('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/bacterial_metabonlite_intersectionLIPIDS.pdf',width=9,height=20)
b+a
dev.off()
...
pdf('~/testLIPIDS.pdf',width=40,height=40)
Heatmap(tmp)
dev.off()
...
ggsave("~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/diet_metabolite_volcano_cem_LIPIDS.pdf", width=18, height=18)
