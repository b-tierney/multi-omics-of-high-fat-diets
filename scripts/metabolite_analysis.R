...
regress_microbe_metabolite2 <- function(m,data_p,data_m,minval){
  ...
  X <- for_regression %>% select(-Metabolite,-ValueLog10,-SampleID,-Type,-Diet) %>% mutate(SEX = as.numeric(as.factor(SEX))-1)
...
regress_microbe_metabolite3 <- function(m,data_p,data_m,minval){
  ...
  for_regression = inner_join(data_m_sub,data_p_sub,by='SampleID')
 # for_regression[is.na(for_regression)] = minval
  output = purrr::map(colnames(for_regression)[8:ncol(for_regression)], function(x) glm(data=for_regression,ValueLog10 ~ for_regression[,x]) %>% tidy%>% filter(term!='(Intercept)')%>% mutate(SGB = x,Metabolite = m)) %>% bind_rows() %>% select(-term)
  return(output)
}
...
merged_data = merged_data %>% filter(SGB %in% (merged_data %>% reshape2::dcast(SGB ~ UID,value.var = 'ABUNDANCE') %>% column_to_rownames('SGB') %>% rowSums %>% data.frame %>% filter(.!=0) %>% rownames))
...
### LOAD METABOLITES
metabolites = read.csv('data_packet/metabolites/FecalMetabolite_logTransformed_long.txt',sep='\t') %>% filter(Time == 'M7') %>% select(-Time) %>% mutate(rawval = 10^(ValueLog10))
metabolites$SampleID = gsub('_M7_1_21_2020','',metabolites$SampleID )
metabolites$SampleID = gsub('_M7_1_22_2020','',metabolites$SampleID )
metabolites$SampleID = gsub('_M7_1_23_2020','',metabolites$SampleID )
...
merged_data_cd_wide = dcast(data = metabolites2 ,Diet + SampleID ~ Metabolite,value.var = 'ValueLog10')

library(vegan)

temp1 = merged_data_cd_wide  %>% mutate(rn = paste(Diet,SampleID,sep='---'))%>% column_to_rownames("rn") %>% select(-Diet,-SampleID)
temp2 = merged_data_cd_wide %>% mutate(rn = paste(Diet,SampleID,sep='---'))%>% column_to_rownames("rn") %>% select(Diet)
...
ggplot()+geom_point(data = site_scores %>% data.frame,size=2, aes(x = dbRDA1, y = dbRDA2,color=DIET)) +theme_minimal() + labs(title = "Fecal Metabolites", x = "RDA1", y = "RDA2")+ theme(legend.position = 'bottom') + geom_segment(data = biplot_scores %>% data.frame,size=.5, aes(x = 0, y = 0, xend = dbRDA1*1, yend = dbRDA2*1,color = var), arrow = arrow(length = unit(0.1, "inches")))+stat_ellipse(data = site_scores %>% data.frame, aes(x = dbRDA1, y = dbRDA2,color = DIET)) + scale_color_brewer(palette = 'Set1') + scale_color_manual(values = colorsDiet)
ggsave('~/HMS Dropbox/Braden Tierney/mouse_diet_semir/plots/biplot_dbrda_FECMET.pdf',width=6,height=6)
...
write.csv(output,'association_output/metabolic_pathway_regression_output_diet.csv')
...
date()
print('Finished linear modeling...')
#write.csv(output,'association_output/metabolic_metaphlan4_regression_output_diet.csv')
...
#write.csv(all_output,'association_output/metabolic_metaphlan4_regression_output_lasso.csv')

date()
#output = purrr::map(unique(metabolites2$Metabolite), function(x) regress_microbe_metabolite3(x,merged_data_tpm_avg %>% mutate(PATHWAY = SGB) ,metabolites2,minval)) 
#output2 = purrr::map(output, function(x) if(typeof(x)!='character'){return(x)})
#output2 = output2[which(!sapply(output2, is.null))]
...
#write.csv(output2,'association_output/metabolic_metaphlan4_regression_output_pairwise.csv')

#### PLOT METABOLITE OUTPUT BY DIET VOLCANO
res = read.csv('association_output/metabolic_metaphlan4_regression_output_diet.csv') %>% mutate(direction = if_else(estimate>0,'positive',if_else(estimate<0,'negative',NA)))
...
class = read.csv('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/FIGURES AND TABLES/metabolite_classifications.csv') %>% dplyr::rename(variable = Metabolite) %>% distinct

res = left_join(res,class)
...
ggsave('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/METABOLITE_diet_metabolite_association_counts.pdf',width=3,height=2.5)
...
#pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/METABOLITE_upset_diet.pdf',width=6,height=4)
...
#### SOME SUMMARY STATS BY CATEGORY
ressig1 = res %>% filter(BH<0.05) %>% filter(Classification != 'Plant Metabolism')
# by group
ressig2 = ressig1 %>% filter(!is.na(Classification))%>% group_by(term,Classification,direction) %>% summarise(n=n()) %>% mutate(n = if_else(direction == 'negative',-1*n,n))%>% dcast(Classification + direction ~ term,value.var = 'n') #%>% column_to_rownames('Classification')
ressig2[is.na(ressig2)] = 0

forhm = ressig2 %>% select(-Classification,-direction)

col_fun <- colorRamp2(c(-50, -3,0, 3,50), rev(c('#BE1E2D','pink', "white",'lightblue', 'darkblue')))

row_ha = rowAnnotation(Direction = ressig2$direction)

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metabolite_metabolisms.pdf',width=6,height=6)
Heatmap(forhm,row_split = ressig2$Classification,row_title_rot = 0,col = col_fun)
dev.off()

# just lipid features by counts
ch <- ressig1 %>%  filter(Classification == 'Lipid Metabolism') %>%   group_by(term, direction, Chain.Length) %>%  summarise(n = n(), .groups = 'drop') %>%  dplyr::rename(var2 = Chain.Length)  %>%  tidyr::complete(term, direction, var2, fill = list(n = 0))%>%   mutate(group = "Chain Length") %>% filter(var2!='')

sat <- ressig1 %>%  filter(Classification == 'Lipid Metabolism') %>%   group_by(term, direction, Saturation) %>%  summarise(n = n(), .groups = 'drop') %>%  dplyr::rename(var2 = Saturation)  %>%  tidyr::complete(term, direction, var2, fill = list(n = 0))%>% mutate(group = "Saturation")%>% filter(var2!='')

satch = bind_rows(sat,ch)
...
satch$term[satch$term == 'FishOil'] = 'Fish Oil'
satch$term[satch$term == 'OliveOil'] = 'Olive Oil'
satch$term[satch$term == 'CoconutOil'] = 'Coconut Oil'
satch$term[satch$term == 'PalmOil'] = 'Palm Oil'
satch$term[satch$term == 'MilkFat'] = 'Milkfat'

satch$var2 = factor(satch$var2, levels = c('Very Long','Long','Medium','Short','Saturated','Polyunsaturated','Monounsaturated','Mixed Saturation'))
satch$direction = factor(satch$direction,levels = c('positive','negative'))

satch <- satch %>% mutate(n = ifelse(is.na(n), 0, n))

ggplot(data = satch %>% filter(var2!='',!is.na(var2)),aes(x = var2,y=n+1,fill=term)) + geom_bar(stat = 'identity',position ='dodge') + cowplot::theme_cowplot() + scale_fill_manual(values = colorsDiet) + facet_grid(direction ~ group,scales = 'free_x') + theme(axis.text.x = element_text(angle = 60,hjust = 1)) + scale_y_log10()
ggsave('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/sat_chain_length.pdf',width=6,height=4)

### steroids

st <- ressig1 %>%  filter(Classification == 'Steroid Metabolism') %>%  group_by(term, direction,variable) %>% dcast(variable ~ term,value.var = 'estimate') %>% column_to_rownames('variable')
st[is.na(st)] = 0


col_fun <- colorRamp2(c(-2,0,2), rev(c('#BE1E2D',"white",'darkblue')))
pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/steroid_betas.pdf',width=5,height=3)
Heatmap(st,col=col_fun)
dev.off()
...
col_fun = circlize::colorRamp2(c(-10,0,10), rev(c('#BE1E2D',"white",'darkblue')))

merged_data_cd_sub_wide_anno_tmp = left_join(merged_data_cd_sub_wide,class%>% dplyr::rename(Metabolite= variable) %>% select(Metabolite,Chain.Length,Saturation,Classification,Sphingolipid))
merged_data_cd_sub_wide_anno = left_join(merged_data_cd_sub_wide,class%>% dplyr::rename(Metabolite= variable) %>% select(Metabolite,Chain.Length,Saturation,Classification))

tmp = merged_data_cd_sub_wide_anno %>% select(Classification) %>% table %>% data.frame %>% filter(Freq>=5) %>% select("Classification") %>% unlist %>% unname %>% as.character

merged_data_cd_sub_wide_anno = merged_data_cd_sub_wide_anno %>% mutate(Classification = if_else(Classification %in% tmp,Classification,'Other'))

chain_length_colors <- brewer.pal(n = 4, name = "Blues") # Using a blue color palette
names(chain_length_colors) <- c("Short", "Medium", "Long", "Very Long")
saturation_colors <- brewer.pal(n = 4, name = "Set3") # Using a pastel color palette
names(saturation_colors) <- c("Saturated", "Monounsaturated", "Polyunsaturated", "Mixed Saturation")
classification_colors <- brewer.pal(n = 12, name = "Paired") # Using a paired color palette for multiple levels
classification_colors <- c(classification_colors, "grey") # Add one more color for "Other"
names(classification_colors) <- c("Lipid Metabolism", "Amino Acid Metabolism", "Carbohydrate Metabolism", 
                                  "Detoxification Metabolism", "Vitamin Metabolism", "Nucleotide Metabolism", 
                                  "Steroid Metabolism", "Polyphenol Metabolism", "Bile Acid Metabolism", 
                                  "Neurotransmitter Metabolism", "Antioxidant Metabolism", "Heme Metabolism", "Other")

col <- list(
  `Chain Length` = chain_length_colors,
  Saturation = saturation_colors,
  Classification = classification_colors
)

merged_data_cd_sub_wide_anno = merged_data_cd_sub_wide_anno %>% arrange(Classification,Saturation,Chain.Length)

col_ha <- columnAnnotation(
  `Chain Length` = merged_data_cd_sub_wide_anno$Chain.Length,
  Saturation = merged_data_cd_sub_wide_anno$Saturation,
  Classification = merged_data_cd_sub_wide_anno$Classification,
  col = col
)

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metabolite_heatmap_unlabeled_unclustered.pdf',width=12,height=4)
Heatmap(merged_data_cd_sub_wide_anno %>% select(-Metabolite,-Chain.Length,-Saturation,-Classification) %>% t,col=col_fun,bottom_annotation = col_ha,cluster_columns = F)
dev.off()


pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metabolite_heatmap_unlabeled_clustered.pdf',width=12,height=4)
Heatmap(merged_data_cd_sub_wide_anno %>% select(-Metabolite,-Chain.Length,-Saturation,-Classification) %>% t,col=col_fun,bottom_annotation = col_ha,cluster_columns = T)
dev.off()
...
merged_data_cd_sub_wide_anno = merged_data_cd_sub_wide_anno_tmp

### FUNCTION TO MAKE HEATMAPS ONE AT A TIME

# Define the modified function
create_heatmap_for_classification <- function(classification_level, data, col_fun, col,
                                              remove_chain_length = FALSE, remove_saturation = FALSE,
                                              show_column_names = TRUE) {
  # Subset the data
  subset_data <- data %>% filter(Classification == classification_level) %>% select(-Sphingolipid)
  
  # Define the annotations
  annotation_data <- list()
  if (!remove_chain_length) {
    annotation_data[["Chain Length"]] <- subset_data$Chain.Length
  }
  if (!remove_saturation) {
    annotation_data[["Saturation"]] <- subset_data$Saturation
  }
  
  # Ensure annotations are named correctly
  if (length(annotation_data) > 0) {
    row_ha <- do.call(rowAnnotation, c(annotation_data, list(col = col)))
  } else {
    row_ha <- NULL
  }
  
  # Select the data for the heatmap, excluding annotations
  data_for_heatmap <- subset_data %>% 
    select( -Chain.Length, -Saturation, -Classification) %>% 
    column_to_rownames("Metabolite")
  
  # Create the heatmap with transposed data
  heatmap <- Heatmap(
    (data_for_heatmap),  # Transpose the data
    col = col_fun,
    right_annotation = row_ha,  # Use right_annotation instead of bottom_annotation for transposed heatmap
    cluster_rows = TRUE,        # Since data is transposed, cluster rows (which are original columns)
...
# Define the modified function
create_heatmap_for_classification2 <- function(classification_level, data, col_fun, col,
                                              remove_chain_length = FALSE, remove_saturation = FALSE,
                                              show_column_names = TRUE) {
  # Subset the data
  subset_data <- data %>% filter(Classification == classification_level) %>% select(-Sphingolipid)
  ...
  heatmap <- Heatmap(
    ...
    cluster_rows = TRUE,        # Since data is transposed, cluster rows (which are original columns)
    show_column_names = show_column_names,
    row_split = subset_data$Chain.Length,
    name = classification_level
  )
...
heatmap_lipid_metabolism_all <- create_heatmap_for_classification2(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'No'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation = FALSE,    # Include saturation annotation
  show_column_names = TRUE      # Show column names
)

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/lipid_metabolism_nosphingos.pdf',width=6,height=20)
heatmap_lipid_metabolism_all
dev.off()


heatmap_lipid_metabolism_sphingos <- create_heatmap_for_classification2(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'Yes'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation =  FALSE,    # Include saturation annotation
...
pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/lipid_metabolism_sphingos.pdf',width=6,height=15)
heatmap_lipid_metabolism_sphingos
dev.off()



# Example usage for one classification level
heatmap_lipid_metabolism_short <- create_heatmap_for_classification(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'No',Chain.Length == 'Short'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation = FALSE,    # Include saturation annotation
...
heatmap_lipid_metabolism_m <- create_heatmap_for_classification(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'No',Chain.Length == 'Medium'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation = FALSE,    # Include saturation annotation
...
heatmap_lipid_metabolism_l <- create_heatmap_for_classification(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'No',Chain.Length == 'Long'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation = FALSE,    # Include saturation annotation
...
heatmap_lipid_metabolism_vl <- create_heatmap_for_classification(
  classification_level = "Lipid Metabolism",
  data = merged_data_cd_sub_wide_anno %>% filter(Sphingolipid == 'No',Chain.Length == 'Very Long'),
  col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
  col = col,
  remove_chain_length = FALSE,  # Include chain length annotation
  remove_saturation = FALSE,    # Include saturation annotation
...
# To generate heatmaps for all classification levels:
classification_levels <- unique(merged_data_cd_sub_wide_anno %>% 
                                  select(Classification) %>% 
                                  filter(Classification != 'Lipid Metabolism') %>% 
                                  unlist() %>% 
                                  unname())

heatmaps_list <- lapply(classification_levels, function(level) {
  create_heatmap_for_classification(
    classification_level = level,
    data = merged_data_cd_sub_wide_anno,
    col_fun = col_fun,  # Assuming col_fun is defined elsewhere for the heatmap colors
    col = col,
    remove_chain_length = TRUE,  # Customize per requirement
    remove_saturation = TRUE,    # Customize per requirement
    show_column_names = TRUE      # Customize per requirement
  )
})


pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/amino_acid.pdf',width=6,height=10)
heatmaps_list[[1]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/antiox.pdf',width=6,height=2.5)
heatmaps_list[[2]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/bilac.pdf',width=6,height=6)
heatmaps_list[[3]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/carb.pdf',width=6,height=6)
heatmaps_list[[4]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/detox.pdf',width=6,height=2.5)
heatmaps_list[[5]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/heme.pdf',width=6,height=2.5)
heatmaps_list[[6]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/neurot.pdf',width=6,height=2.5)
heatmaps_list[[7]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/nucleo.pdf',width=6,height=2.5)
heatmaps_list[[8]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/polyphen.pdf',width=6,height=4)
heatmaps_list[[10]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/ster.pdf',width=6,height=3)
heatmaps_list[[11]]
dev.off()

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metaboliteheatmaps/vit.pdf',width=6,height=3.5)
heatmaps_list[[12]]
dev.off()
...
metphaphlanlasso = read.csv('association_output/metabolic_metaphlan4_regression_output_lasso.csv',check.names=F) %>% melt() 
...
bacmostmet = left_join(bacmostmet,class)

bacmostmet = bacmostmet %>% select(Species,Classification,direction) %>% group_by(Species,Classification,direction) %>% table %>% data.frame 

metsub = bacmostmet%>% mutate(Species = gsub('s__','',Species)) %>% dplyr::group_by(Classification,direction)%>% dplyr::filter(Freq>0)%>% arrange(desc(Freq)) %>% ungroup %>% dplyr::group_by(Species) %>% mutate(sumval = sum(Freq))  %>% mutate(Classification = if_else(!(Classification %in% c('Alkaloid Metabolism','Amino Acid Metabolism','Polyamine Metabolism','Heme Metabolism','Plant Metabolism','Detoxification Metabolism')),Classification,'Other Metabolism'))  
...
ggplot(data = metsub %>% filter(Freq>1),aes(x = fct_reorder(Species,sumval),y = Freq+1,fill=Classification)) + geom_bar(stat='identity') + cowplot::theme_cowplot() + theme(axis.title.x =  element_blank(),axis.text.x = element_text(angle=60,hjust=1)) + ggtitle('Bacteria with the most fecal metabolomic associations') +scale_fill_brewer(palette = 'Set3') + facet_grid(direction ~ .) + scale_y_log10() + theme(legend.position = 'bottom',legend.title = element_blank())#+scale_fill_manual(values = c('darkblue','#BE1E2D')) 
ggsave('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/bacteria_with_most_metabolites_metaphlan4.pdf',width=16,height=5)

### load pairwise microbe–metabolite regression output before downstream plotting
bacregout = read.delim('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/association_output/metabolic_metaphlan4_regression_output_pairwise.csv',sep=',')  %>% mutate(BH = p.adjust(p.value))
...
bacmostmet2 = bacregout %>% filter(Species!='s__') %>% mutate(BH=p.adjust(p.value))%>% filter(BH<0.05) %>% mutate(direction = if_else(estimate>0,'positive',if_else(estimate<0,'negative',NA))) %>% group_by(direction) %>% filter(estimate!=0)  %>% select(Species) %>% table %>% data.frame 
...
ggsave('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/bacteria_with_most_metabolites_metaphlan4_FDR.pdf',width=10,height=4)


#metphaphlanlasso  = metphaphlanlasso %>% select(-all_of(colnames(metphaphlanlasso)[grepl("^X-", colnames(metphaphlanlasso))]))
#pdf('~/HMS Dropbox/Braden Tierney//Super Mice Diet Omics Project/Metagenomics/Metabolomic_Associations_Nov23/metabolite_microbe_heatmap.pdf',width=16,height=10)
#Heatmap(metphaphlanlasso)
#dev.off()
#png('~/HMS Dropbox/Braden Tierney//Super Mice Diet Omics Project/Metagenomics/Metabolomic_Associations_Nov23/metabolite_microbe_heatmap.png')
#Heatmap(metphaphlanlasso)
#dev.off()
...
col_fun1 = circlize::colorRamp2(c(-15,5,0,5,15), c("#d73027",'darkblue','#000000','orange',"#4575b4"))

pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/metabolite_diet_associations_hm.pdf',width=5,height=12)
...
dietsub = diet %>% filter(BH<0.05) %>% filter(grepl('s__', GTDB_TAX) & !grepl('s__$', GTDB_TAX)) %>% mutate(microbe = strsplit(GTDB_TAX, 's__') %>% map_chr(2))%>% mutate(diet = term)
diettimesub = longtax %>% filter(grepl(':',term)) %>% mutate(term = gsub('TIME_RECODE_OVERALL:','Time X ',term)) %>% filter(BH<0.05) %>% filter(grepl('s__', GTDB_TAX) & !grepl('s__$', GTDB_TAX)) %>% mutate(microbe = strsplit(GTDB_TAX, 's__') %>% map_chr(2))  %>% mutate(diet = strsplit(term,'X ') %>% map_chr(2))
...
bacmetsub2 = bacregout %>% mutate(Species = gsub('s__','',Species)) %>% filter(!is.na(Species),Species!='') %>% filter(Species %in% tokeep)%>% filter(Metabolite %in% rownames(metabolites3)) %>% mutate(BH = p.adjust(p.value))  %>% filter(BH<0.05) 
...
pdf('~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/bacterial_metabonlite_intersection.pdf',width=9,height=20)
b+a
dev.off()
...
pdf('~/test.pdf',width=40,height=40)
Heatmap(tmp)
dev.off()
...
ggsave("~/HMS Dropbox/Braden Tierney//mouse_diet_semir/plots/diet_metabolite_volcano_cem.pdf", width=18, height=18)
